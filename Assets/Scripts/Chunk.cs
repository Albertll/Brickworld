...
namespace Assets.Scripts
{
    ...
    public class Chunk
    {
        ...
        public HeightBricks[][] Bricks { get; }

        public static int HeightSacale = 20;
        public static float DetailScale = 25.0f;
        ...
        public Chunk(int a, int b)
        {
            ...
            Bricks = new HeightBricks[ChunkSize][];
            for (var i = 0; i < ChunkSize; i++)
            {
                Bricks[i] = new HeightBricks[ChunkSize];
                for (var j = 0; j < ChunkSize; j++)
                {
                    var y = GetMapHeight(a * ChunkSize + i, b * ChunkSize + j);

                    Bricks[i][j] = new HeightBricks(y);
                }
            }
        }

        public static int GetMapHeight(int x, int y)
        {
            //var seed = (int)Network.time * 10;//3501720
            var seed = 195084;
            ////return (int)(Mathf.PerlinNoise((x + seed) / DetailScale, (y + seed) / DetailScale) * HeightSacale);
            return (int)((Perlin.perlin((x + seed) / DetailScale, (y + seed) / DetailScale, 0) * HeightSacale)
                          + (Perlin.perlin((x + seed) / DetailScale / 10, (y + seed) / DetailScale / 10, 0) *
                             HeightSacale));
            //return 0;
        }
    }
